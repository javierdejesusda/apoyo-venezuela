/**
 * Reverse the vzlayuda.com import, SCOPED to that source only. Deletes every
 * location whose source_ref starts with 'vzlayuda:'; needs attached to those
 ...
 *
 * Usage:
 *   node --env-file=.env.local scripts/rollback-vzlayuda.mjs        # dry run
 *   node --env-file=.env.local scripts/rollback-vzlayuda.mjs --yes  # delete
 */
import { createClient } from '@supabase/supabase-js';

import { SOURCE_PREFIX } from './vzlayuda-transform.mjs';
...
async function main() {
  ...
  console.error(`vzlayuda locations to delete: ${locs.length} (needs ~${needCount}, cascade)`);
