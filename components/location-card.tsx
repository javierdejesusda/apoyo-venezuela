...
export function LocationCard({ location }: { location: LocationWithNeeds }) {
  ...
  return (
    <Link
      ...
      className="group block rounded-2xl border border-border bg-surface p-4 transition-colors hover:border-brand-300 focus-visible:border-brand-400"
    >
      ...
      {categories.length > 0 && (
        <div className="mt-3 flex flex-wrap items-center gap-1.5">
          ...
          {openNeeds.length > categories.length && (
            <span className="text-xs text-ink-faint">+{openNeeds.length - categories.length} más</span>
          )}
        </div>
      )}
      ...
      <div className="mt-3 flex items-center justify-between border-t border-border pt-3 text-xs text-ink-faint">
        <span>
          <span className="font-medium text-ink-soft">{summary.total}</span> necesidades
          {summary.urgentes > 0 && <span className="text-danger"> · {summary.urgentes} urgentes</span>}
        </span>
